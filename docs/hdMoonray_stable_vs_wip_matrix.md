...
| Render Settings LOP | Partial / WIP | `moonray_dcc_plugins` `8c995f8` through `6cd0361` | `moonray_render_settings.py`, generated HDA, `moonray_render_settings_lop_audit.md`, validation script | Source-generated HDA, USD RenderSettings/Product foundation, optional RenderVar diagnostics, and owned USD Render ROP lifecycle are useful. AOV integration and final UX remain unsettled. | Good process evidence for source-of-truth HDA generation; not yet a stable renderer-settings pattern. Current Track A default keeps normal beauty on the no-Beauty-RenderVar path: `RenderProduct` with `productName`, `productType = raster`, and empty `orderedVars`. Beauty RenderVar and non-beauty AOVs remain experimental/debug until filled render or EXR proof exists. |
| Beauty buffer / USD Render ROP support | Partial / WIP | hdMoonray `2eb0808` | `ArrasRenderer.cc`, `UsdRenderers.json` | Beauty path is useful progress, but non-beauty AOVs remain blocked. | Do not claim full image-buffer/AOV support from this commit. Authored Beauty RenderVar is an experimental/debug path in the custom LOP because it can force Hydra AOV/RenderBuffer lifecycle behavior. Normal beauty rendering currently defaults to no authored Beauty RenderVar until viewport/IPR, USD Render ROP/husk, and filled output proof justify a different default. |
